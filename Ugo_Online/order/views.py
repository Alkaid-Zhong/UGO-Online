        ...
        if serializer.is_valid():
            ...
            for order in orders:
                ...
                for seller in sellers:
                    new_message(seller, f'[{order.shop.name}] 用户 {order.user} 已支付订单 {order.id}，请尽快发货！', order.id)
...
class CancelOrderView(APIView):
    ...
    def post(self, request, order_id):
        ...
        for seller in sellers:
            new_message(seller, f'[{order.shop.name}] 用户 {order.user} 取消了订单 {order.id}！', order.id)
...
class RefundOrderItemsView(APIView):
    ...
    def post(self, request, order_id):
        ...
        for item in order_items:
            ...
            for seller in sellers:
                new_message(seller, f'[{order.shop.name}] 用户 {order.user} 退回了订单 {order.id} 中的商品项 {item.product}！', order.id)
...
class UpdateOrderStatus2ShippedView(APIView):
    ...
    def post(self, request, order_id):
        ...
        new_message(order.user, f'您的订单 {order.id} 已发货，请在收到货后确认收货！', order.id)
...
class UpdateOrderAddressView(APIView):
    ...
    def post(self, request, order_id):
        ...
        if serializer.is_valid():
            ...
            for seller in sellers:
                # print(seller)
                new_message(seller, f'[{order.shop}] 订单 {order.id} 的地址已修改为 "{address}"！', order.id)
